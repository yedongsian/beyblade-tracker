...
> **本檔是唯一的 backlog；優先順序與「新回饋要不要插隊」的判斷準則見 [`ROADMAP.md` §3](ROADMAP.md#3-下一階段2026-09-04-起)。**
...
### BT-UX-009 — 更新成功時顯示「Failed to fetch」，看起來像失敗

- Priority：P1
- Status：Fixed（程式已修，需 1.0.5 之後的版本才會到使用者手上）
- 2026-09-04 VM 實測 1.0.4 → 1.0.5 時出現。設定頁紅字顯示 `Failed to fetch`。
- **這不是失敗，是成功的必然結果。** 更新完成時舊服務會被停掉換成新版 ——
  而瀏覽器正在輪詢 `/api/update/progress` 的，就是那個即將消失的舊服務。
  連線一定會斷。`pollUpdate` 的 catch 把它當成錯誤處理，於是：

  1. 在更新**真的成功**的那一刻，對使用者說「失敗」，引誘他再跑一次；
  2. 顯示的是瀏覽器原生字串 `Failed to fetch` —— 未翻譯、沒有錯誤代碼、沒有處置方式。
     這個專案有一整套錯誤代碼系統，就是為了不讓這種東西出現在使用者面前。

- 修法：輪詢記住最後一次已知的階段。若連線在 `installing` / `verifying` / `completed`
  期間斷掉，那是交接的一部分 —— 改為顯示「更新已完成，正在重新啟動服務…」，
  並輪詢 `/health` 直到新服務回應後重新載入頁面。
  重新載入順帶讓使用者直接看到新版本號（`BT-UX-005` 在這條路徑上的症狀也一併消失）。
- `checking` / `downloading` 階段的失敗仍照原本方式回報，這有測試守住。
...
- **2026-09-04 補充：同一個成因還有第二個症狀。** 更新到 1.0.3 完成後，設定頁的
  「版本與執行環境」仍顯示 `版本：1.0.1`，**F5 重新整理後才變成 1.0.3**。
  版本更新卡片有正確收起（那塊有 `settingsScript` 在輪詢 `/api/update/status`），
  但版本字串是伺服器端渲染的靜態 HTML，沒有任何東西回頭更新它。
- 所以這張工單的本質不是「橫幅出現得太晚」，而是**已載入的頁面不會反映後來的變化**。
  更新完成後最該讓使用者確認的就是版本號，而它偏偏是不會動的那個。
- 使用者 2026-09-04 判定為小事，同意延後到下次有更新時一併處理。
...
- **2026-09-04 已修（1.0.5）。** 日期依介面語言格式化；`#update-details` 加上
  `white-space:pre-line`，四段資訊才真的分行；發佈說明若只是重複「發行者 版本」則略過。
  日期無法解析時只丟掉日期，不會讓整張卡片壞掉。
- 測試是**實際執行產出的用戶端函式**，不是比對原始碼字串：三種語言的輸出、
  重複發佈說明、壞日期各一項，並用渲染後的頁面確認 CSS 規則在。
...
- **2026-09-04 已修（1.0.4）。** 實際範圍比原本記的大：`manifestUrl` 與 `publicKey` **兩者**
  都只來自環境變數且預設為空，所以一般使用者不只驗不了更新，是**根本不會去檢查**。
- 建置流程把兩者寫進產物的 `release.json`；`config.js` 改為「環境變數 → 產物 → 空」三段式。
- 公鑰**從簽章私鑰導出**（`createPublicKey`），不另外指定公鑰檔 —— 配錯的症狀是 `BT-UPD-003`，
  與「沒設公鑰」無法區分。導出則結構上不可能配錯。
- 更新網址用 `/releases/latest/download/`，不寫死版本；否則每個版本只會檢查到自己。
  **副作用：取消某個發佈的 prerelease 標記，就等於把它推送給所有使用者。**
- 已驗（本機）：產物自帶的公鑰能驗過同一次建置的 manifest；在完全無環境變數下
  `config.update` 正確取得兩個值；環境變數仍可覆寫。
- **2026-09-04 VM 驗證通過。** 清除環境變數（登錄檔與行程兩個範圍）後安裝 1.0.4：

  | 來源 | 值 |
  | --- | --- |
  | 行程／使用者／機器環境變數 | 全空 |
  | `.env` | 不存在 |
  | 產物 `release.json` | 內建網址與公鑰皆有 |
  | `/health` 實際使用的更新來源 | `.../releases/latest/download/release-manifest.json` |

- 過程中一度誤判為通過：畫面顯示 1.0.4 與「目前已是最新版本。」，看起來完全正常，
  但 `/health` 回報的更新來源仍是上一輪環境變數裡的 v1.0.3 網址。
  成因是 `SetEnvironmentVariable('User')` 只改登錄檔，**已在執行的行程保有自己的副本** ——
  選單視窗在清除前就開著，安裝器與它啟動的服務一路繼承了舊值。
  抓到它的是腳本裡比對「更新來源是否為 `/releases/latest/download`」那一行；
  人會看的每一個地方都正常。
