...
function renderHealth(data){const health=Object.hasOwn(data,'health')?data.health:updateState.health;const rollback=Object.hasOwn(data,'rollback')?data.rollback:updateState.rollback;if(!updateHealth)return;if(health?.rollbackOffered){updateHealth.textContent=message('updateHealthFailed',{code:health.code||'BT-UPD-006'});updateRollback.hidden=false}else if(rollback?.status==='failed'){updateHealth.textContent=message('rollbackFailed',{code:rollback.code||'BT-UPD-007'});updateRollback.hidden=true}else if(rollback?.status==='succeeded'){updateHealth.textContent=settingsMessages.rollbackSucceeded;updateRollback.hidden=true}else{updateHealth.textContent='';updateRollback.hidden=true}}
function renderUpdate(data){updateState=data.state||updateState;availableUpdate=data.manifest||null;const deferred=Boolean(data.deferred);if(!data.updateAvailable||!availableUpdate){updateDetails.textContent=settingsMessages.noUpdate;updateApply.hidden=true;updateDefer.hidden=true;renderHealth(data);return}updateDetails.textContent=message(deferred?'updateDeferred':'updateAvailable',{version:availableUpdate.version})+'\n'+availableUpdate.publisher+' · '+bytes(availableUpdate.size)+' · '+availableUpdate.publishedAt+'\n'+availableUpdate.releaseNotes;updateApply.hidden=deferred;updateDefer.hidden=deferred;renderHealth(data)}
if(availableUpdate)renderUpdate({updateAvailable:true,manifest:availableUpdate,deferred:Boolean(updateState.deferred),health:updateState.health,rollback:updateState.rollback});setTimeout(async()=>{try{renderUpdate(await api('/api/update/status',{method:'GET'}))}catch{/* status is optional while the service starts */}},1000);setInterval(async()=>{try{renderUpdate(await api('/api/update/status',{method:'GET'}))}catch{/* keep the last verified result */}},60000);
async function pollUpdate(id){updateProgress.hidden=false;const timer=setInterval(async()=>{try{const data=await api('/api/update/progress/'+id,{method:'GET'});const percent=data.total?Math.min(100,Math.round(data.received/data.total*100)):0;updateProgress.value=percent;updateDetails.textContent=data.phase==='downloading'?message('updateDownloading',{percent}):data.phase==='installing'?settingsMessages.updateInstalling:data.phase==='completed'?settingsMessages.updateRestarting:data.error||settingsMessages.updateFailed;if(['completed','failed'].includes(data.phase)){clearInterval(timer);updateProgress.hidden=true;setSettingsStatus(data.phase==='completed'?settingsMessages.updateRestarting:data.error,data.phase==='completed'?'success':'error')}}catch(error){clearInterval(timer);updateProgress.hidden=true;setSettingsStatus(error.message,'error')}},400)}
...
updateApply?.addEventListener('click',async()=>{if(!availableUpdate)return;if(!confirm(message('updateConfirm',{version:availableUpdate.version})))return;setSettingsStatus(settingsMessages.checking);try{const data=await api('/api/update/apply',{method:'POST',body:JSON.stringify({confirmed:true,targetVersion:availableUpdate.version,manifestDigest:availableUpdate.manifestDigest})});updateApply.hidden=true;updateDefer.hidden=true;pollUpdate(data.operationId)}catch(error){setSettingsStatus(error.message,'error')}});
