...
| `BT-SRC-004` | 要探索的商店已不存在。 | 重新整理來源管理頁；若商店已刪除請重新加入。 |
| `BT-SRC-005` | 這間商店已有探索工作正在執行。 | 等待完成即可；探索需數分鐘，不必重複按。這是預期行為。 |
| `BT-SRC-006` | 這間商店沒有可供探索的網址。 | 到來源管理加入一個分類頁或商品頁。 |
| `BT-SRC-007` | 探索網址不在該商店的網域內。 | 改用同一網域下的網址；跨網域探索會被安全邊界擋下。 |
