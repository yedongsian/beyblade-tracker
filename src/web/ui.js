...
function renderUpdate(data){if(activeUpdateOperation)return;updateState=data.state||updateState;availableUpdate=data.manifest||null;const deferred=Boolean(data.deferred);if(!data.updateAvailable||!availableUpdate){updateDetails.textContent=data.enabled===false?settingsMessages.updateUnavailable:settingsMessages.noUpdate;updateApply.hidden=true;updateDefer.hidden=true;updateResume.hidden=true;renderHealth(data);return}updateDetails.textContent=message(deferred?'updateDeferred':'updateAvailable',{version:availableUpdate.version})+'\\n'+availableUpdate.publisher+' · '+bytes(availableUpdate.size)+' · '+availableUpdate.publishedAt+'\\n'+availableUpdate.releaseNotes;updateApply.hidden=deferred;updateDefer.hidden=deferred;updateResume.hidden=!deferred;renderHealth(data)}
