...
訊息已改為三語可操作版本（`BT-UX-003`），需要一個**加得進去、但掃描時會失敗**的來源。

> **不要用 D-2 那個已下架的 shimamura URL。** 2026-08-11 實測：它在**預覽**階段就以
> 「網站重新導向超過 3 次。」被擋下，加不進去；而那句話來自預覽路徑，本來就是中文的，
> 測不到本項要驗的東西。BT-UX-003 管的是加入之後、**掃描時**的失敗。

做法：

1. 加入 `https://www.hlj.com/product/TKT09613`，確認掃描成功
2. **中斷網路**（實體斷線、停用網路卡，或 VM 的「取消連接網路卡」）
3. 按該來源的「**立即重新檢查**」（60 秒冷卻）
4. 重新整理來源管理頁，確認：
   - 顯示 **「無法連線到商店。請確認網路連線，稍後會自動重試。」**
   - **不是** `TypeError: fetch failed`
   - 展開「技術細節」**看得到**原始訊息
   - 切換 UI 語言，訊息跟著換
5. 接回網路，再按一次「立即重新檢查」，錯誤應消失
