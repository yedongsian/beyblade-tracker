...
> ⚠ **這一步不要找「每一頁都有的更新橫幅」。** 此刻服務跑的是 1.0.0，
> 而全站橫幅（`availableUpdateBanner`）是 **1.0.1 才引入**的功能（commit `aed1a69`）。
> 在 1.0.0 上它按定義不會出現，只有設定頁看得到更新資訊 —— 那是正確行為。
> 橫幅的驗證移到步驟 7 之後（見「步驟 7b」）。

到設定頁，版本更新區塊應顯示偵測到 **1.0.1**。若沒有，按一次「檢查更新」。
...
**只有在步驟 7 確認服務真的跑 1.0.1 之後才做這一步。** 這是本輪新增的功能：
先前只有設定頁看得到更新，使用者不主動去點就永遠不知道有新版。

（要看到橫幅，需要有一個「更有新版」的目標。若 1.0.1 已是最新，可暫時把
`UPDATE_MANIFEST_URL` 指向一個更高版本的 manifest；沒有的話這一項就留到下次發版驗。）

| 位置 | 應該看到 |
| --- | --- |
| 總覽、商品、來源管理…**任何一頁** | 「**有新版本 X 可用。**」＋「前往設定查看」連結 |

也順便切一次英文，橫幅應變成 `Version X is available.` ＋ `Open Settings`。
