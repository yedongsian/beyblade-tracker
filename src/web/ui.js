...
.source-card{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,40%);gap:1rem;padding:1rem;border:1px solid var(--line);border-radius:14px;background:#fff}
...
.card-status{flex-basis:100%;text-align:right;margin:.35rem 0 0;min-height:0}.card-status:empty{display:none}.pill{display:inline-flex;align-items:center;padding:.2rem .55rem;border-radius:999px;font-size:.78rem;font-weight:700;background:#eef2f8;color:#344054}
...
.notice{border-left:4px solid var(--brand);background:#eef3ff;padding:.75rem 1rem;border-radius:8px;margin:.75rem 0}
/* updateDetails 以 textContent 塞入換行；沒有這條，四段資訊會擠成一句流水帳。 */
#update-details{white-space:pre-line}.notice.warn{border-color:var(--warn);background:#fff8ed}
...
function reportUrl(error){const query=new URLSearchParams({template:'bug_report.yml',title:'[問題回報] '+error.code,error_code:error.code,app_version:error.appVersion});return 'https://github.com/yedongsian/beyblade-tracker/issues/new?'+query}
...
export function settingsScript(t = createTranslator()) {
  const messages = { saved: t('js.saved'), checking: t('settings.checking'), noUpdate: t('settings.noUpdate'), updateUnavailable: t('settings.updateUnavailable'), importConfirm: t('settings.importConfirm'), updateAvailable: t('settings.updateAvailable'), updateDeferred: t('settings.updateDeferred'), updateChecking: t('settings.updateChecking'), updateDownloading: t('settings.updateDownloading'), updateInstalling: t('settings.updateInstalling'), updateVerifying: t('settings.updateVerifying'), updateRestarting: t('settings.updateRestarting'), updateFailed: t('settings.updateFailed'), updateConfirm: t('settings.updateConfirm'), updateVerified: t('settings.updateVerified'), updateHealthFailed: t('settings.updateHealthFailed'), rollbackConfirm: t('settings.rollbackConfirm'), rollbackQueued: t('settings.rollbackQueued'), rollbackSucceeded: t('settings.rollbackSucceeded'), rollbackFailed: t('settings.rollbackFailed') };
  return `
...
const updateCard=document.getElementById('update-card');let availableUpdate=(()=>{try{return JSON.parse(updateCard?.dataset.scheduledUpdate||'null')?.manifest||null}catch{return null}})();let updateState=(()=>{try{return JSON.parse(updateCard?.dataset.updateState||'{}')}catch{return {}}})();let activeUpdateOperation=null;let updatePollTimer=null;let updatePollInFlight=false;const updateDetails=document.getElementById('update-details');const updateHealth=document.getElementById('update-health');const updateProgress=document.getElementById('update-progress');const updateCheck=document.getElementById('update-check');const updateApply=document.getElementById('update-apply');const updateDefer=document.getElementById('update-defer');const updateResume=document.getElementById('update-resume');const updateRollback=document.getElementById('update-rollback');
function setUpdateBusy(busy){for(const button of [updateCheck,updateApply,updateDefer,updateResume]){if(button)button.disabled=busy}if(busy){updateApply.hidden=true;updateDefer.hidden=true;updateResume.hidden=true}}
...
// 2026-08-29 實測畫面：「可更新至 1.0.1。 Beyblade Tracker · 26.2 MB · 2026-08-29T16:05:53.570Z
// Beyblade Tracker 1.0.1」。原始 ISO 時間戳直接給了使用者，換行又被吃掉，五段資訊擠成一句。
// 這個產品的前提是使用者不必碰技術工具，主畫面就不該出現 2026-08-29T16:05:53.570Z。
function updateWhen(value){try{const d=new Date(value);if(isNaN(d))return'';return d.toLocaleDateString(document.documentElement.lang||undefined,{year:'numeric',month:'long',day:'numeric'})}catch{return''}}
function updateSummary(manifest,deferred){const lines=[message(deferred?'updateDeferred':'updateAvailable',{version:manifest.version})];const facts=[manifest.publisher,bytes(manifest.size),updateWhen(manifest.publishedAt)].filter(Boolean);if(facts.length)lines.push(facts.join(' · '));const notes=(manifest.releaseNotes||'').trim();if(notes&&notes!==manifest.publisher&&notes!==manifest.publisher+' '+manifest.version)lines.push(notes);return lines.join('\\n')}
// 更新成功會停掉正在服務這個頁面的行程，所以輪詢一定會斷。把它當成錯誤，就會在更新
// 真的成功時對使用者顯示瀏覽器原生的「Failed to fetch」—— 沒有代碼、沒有處置方式、
// 而且是英文。等服務回來再重新載入，使用者看到的就是新版本的頁面。
function awaitService(deadline){const until=deadline||Date.now()+180000;const tick=async()=>{try{const r=await fetch('/health',{cache:'no-store'});if(r.ok){location.reload();return}}catch{}if(Date.now()<until)setTimeout(tick,2000);else setSettingsStatus(message('updateFailed',{}),'error')};setTimeout(tick,2000)}
function renderUpdate(data){if(activeUpdateOperation)return;updateState=data.state||updateState;availableUpdate=data.manifest||null;const deferred=Boolean(data.deferred);if(!data.updateAvailable||!availableUpdate){updateDetails.textContent=data.enabled===false?settingsMessages.updateUnavailable:settingsMessages.noUpdate;updateApply.hidden=true;updateDefer.hidden=true;updateResume.hidden=true;renderHealth(data);return}updateDetails.textContent=updateSummary(availableUpdate,deferred);updateApply.hidden=deferred;updateDefer.hidden=deferred;updateResume.hidden=!deferred;renderHealth(data)}
async function refreshUpdateStatus(){try{if(activeUpdateOperation)return;const data=await api('/api/update/status',{method:'GET'});renderUpdate(data);if(data.operation)pollUpdate(data.operation.id)}catch{/* keep the last verified result */}}
async function pollUpdate(id){if(activeUpdateOperation&&activeUpdateOperation!==id&&activeUpdateOperation!=='pending')return;if(activeUpdateOperation===id&&updatePollTimer)return;activeUpdateOperation=id;setUpdateBusy(true);updateProgress.hidden=false;let lastPhase=null;const poll=async()=>{if(activeUpdateOperation!==id||updatePollInFlight)return;updatePollInFlight=true;try{const data=await api('/api/update/progress/'+id,{method:'GET'});lastPhase=data.phase;let details=settingsMessages.updateFailed;if(data.phase==='checking'){updateProgress.removeAttribute('value');details=settingsMessages.updateChecking}else if(data.phase==='downloading'){const percent=data.total?Math.min(100,Math.round(data.received/data.total*100)):0;updateProgress.value=percent;details=message('updateDownloading',{percent})}else if(data.phase==='installing'){updateProgress.removeAttribute('value');details=settingsMessages.updateInstalling}else if(data.phase==='verifying'){updateProgress.removeAttribute('value');details=settingsMessages.updateVerifying}else if(data.phase==='completed'){updateProgress.removeAttribute('value');details=settingsMessages.updateRestarting}else if(data.phase==='failed'){updateProgress.removeAttribute('value');details=data.errorCode||settingsMessages.updateFailed}else{updateProgress.removeAttribute('value')}updateDetails.textContent=details;if(['completed','failed'].includes(data.phase)){activeUpdateOperation=null;updatePollTimer=null;updateProgress.hidden=true;if(data.phase==='failed'){setUpdateBusy(false);renderUpdate({updateAvailable:Boolean(availableUpdate),manifest:availableUpdate,state:updateState,deferred:Boolean(updateState.deferred)});setSettingsStatus(data.errorCode||settingsMessages.updateFailed,'error')}else{setSettingsStatus(settingsMessages.updateRestarting,'success')}return}}catch(error){if(['installing','verifying','completed'].includes(lastPhase)){updateProgress.removeAttribute('value');updateDetails.textContent=settingsMessages.updateRestarting;setSettingsStatus(settingsMessages.updateRestarting,'success');awaitService();return}activeUpdateOperation=null;updatePollTimer=null;updateProgress.hidden=true;setUpdateBusy(false);renderUpdate({updateAvailable:Boolean(availableUpdate),manifest:availableUpdate,state:updateState,deferred:Boolean(updateState.deferred)});setSettingsStatus(error.message,'error');return}finally{updatePollInFlight=false}if(activeUpdateOperation===id)updatePollTimer=setTimeout(poll,400)};poll()}
if(availableUpdate)renderUpdate({updateAvailable:true,manifest:availableUpdate,deferred:Boolean(updateState.deferred),health:updateState.health,rollback:updateState.rollback});if(updateState.operation)pollUpdate(updateState.operation.id);setTimeout(refreshUpdateStatus,1000);setInterval(refreshUpdateStatus,60000);
updateCheck?.addEventListener('click',async()=>{if(activeUpdateOperation)return;setSettingsStatus(settingsMessages.checking);try{const data=await api('/api/update',{method:'GET'});renderUpdate(data);setSettingsStatus(data.updateAvailable?settingsMessages.updateVerified:settingsMessages.noUpdate,data.updateAvailable?'success':'')}catch(error){setSettingsStatus(error.message,'error')}});
updateDefer?.addEventListener('click',async()=>{if(activeUpdateOperation||!availableUpdate)return;try{await api('/api/update/defer',{method:'POST',body:JSON.stringify({targetVersion:availableUpdate.version,manifestDigest:availableUpdate.manifestDigest})});renderUpdate({updateAvailable:true,manifest:availableUpdate,deferred:true});setSettingsStatus(message('updateDeferred',{version:availableUpdate.version}),'success')}catch(error){setSettingsStatus(error.message,'error')}});
updateResume?.addEventListener('click',async()=>{if(activeUpdateOperation)return;try{await api('/api/update/resume',{method:'POST',body:'{}'});renderUpdate({updateAvailable:true,manifest:availableUpdate,deferred:false});setSettingsStatus(settingsMessages.saved,'success')}catch(error){setSettingsStatus(error.message,'error')}});
updateApply?.addEventListener('click',async()=>{if(activeUpdateOperation||!availableUpdate)return;if(!confirm(message('updateConfirm',{version:availableUpdate.version})))return;activeUpdateOperation='pending';setUpdateBusy(true);setSettingsStatus(settingsMessages.checking);try{const data=await api('/api/update/apply',{method:'POST',body:JSON.stringify({confirmed:true,targetVersion:availableUpdate.version,manifestDigest:availableUpdate.manifestDigest})});pollUpdate(data.operationId)}catch(error){activeUpdateOperation=null;setUpdateBusy(false);renderUpdate({updateAvailable:true,manifest:availableUpdate,state:updateState,deferred:Boolean(updateState.deferred)});setSettingsStatus(error.message,'error')}});
