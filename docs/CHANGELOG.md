...
格式參考 Keep a Changelog；本專案目前使用 semantic version `1.0.7`。Git 在 2026-07-16 才建立，因此更早的開發歷史只以現存文件為準。
...
- **使用教學隨產物出貨。** `docs/USER_GUIDE.md` 是唯一沒被打包進安裝內容的使用者文件 ——
  它只存在於 repo，任何人裝完之後在自己電腦上都找不到它。現在與其他說明一起放在安裝目錄。

- **更新過時且會誤導的文件。** 產物裡夾帶的說明停在 1.0.0：README 宣稱版本為 `1.0.0`、
  並把 installer 說成「release candidate，HTTPS release channel 驗收尚未完成」——
  更新流程已於 2026-09-04 在 clean VM 完整驗證，這句已不成立；`INSTALL.md` 則寫死了
  `BeybladeTracker-1.0.0-Setup.exe` 這個不存在的檔名。

- **首次安裝的 SmartScreen 警告寫進文件。** 安裝檔尚未購買程式碼簽章憑證，
  第一次安裝一定會出現「Windows 已保護您的電腦」。這發生在使用者看到任何功能之前，
  不事先說明的話，合理的反應就是把檔案當成有問題而刪掉。三份文件現在都在開頭講明。

### 仍未完成

- `BT-UX-001` 程式碼簽章憑證。在購買並簽署之前，SmartScreen 警告會持續出現。

## [1.0.6] — 2026-09-04

修正一個會讓使用者誤以為更新失敗、進而重跑更新的問題。
...
- **BT-UX-009** 更新成功時顯示「Failed to fetch」，看起來像失敗。
  ...
  **這不是失敗，是成功的必然結果。** 更新完成時舊服務會被停掉換成新版 ——
  而瀏覽器正在輪詢 `/api/update/progress` 的，就是那個即將消失的舊服務，連線一定會斷。
  原本的處理把它當成錯誤，於是：

  1. 在更新**真的成功**的那一刻對使用者說「失敗」，引誘他再跑一次；
  2. 顯示的是瀏覽器原生字串 `Failed to fetch` —— 未翻譯、沒有錯誤代碼、沒有處置方式。
     這個專案有一整套錯誤代碼系統，就是為了不讓這種東西出現在使用者面前。

  現在輪詢會記住最後一次已知的階段。若連線在 `installing`／`verifying`／`completed`
  期間斷掉，那是交接的一部分 —— 改為顯示「更新已完成，正在重新啟動服務…」，
  並等新服務回應後重新載入頁面。**順帶讓使用者直接看到新版本號**，
  不必再手動重新整理（`BT-UX-005` 在更新這條路徑上的症狀也一併消失）。

  `checking`／`downloading` 階段的失敗仍照原本方式回報，有測試守住這條界線。

### 已知未修

- `BT-UX-005`（P2）已載入的頁面不會反映後續變化。更新路徑上已由本版順帶解決，
  但其他情境（例如開著頁面時排程檢查發現新版）仍需重新整理。
- `BT-UX-007`（P2）更新**成功**後沒有使用者可及的回滾入口。

### 驗收狀態

更新鏈已於 2026-09-04 在 clean VM 上完整驗證（1.0.4 → 1.0.5，全程未設任何環境變數）。
原始輸出見 `docs/acceptance-evidence/2026-09-04-update-chain/`。

## [1.0.5] — 2026-09-04

兩個在 1.0.4 的 VM 驗收過程中浮現的顯示層問題。

### 修正的缺陷

- **BT-UX-008** 無法檢查更新時，畫面仍宣稱「目前已是最新版本。」

  伺服器端本來就正確渲染「正式更新來源尚未設定。」，但頁面載入 1 秒後被
  `refreshUpdateStatus` 蓋掉 —— `renderUpdate` 在沒有可用更新時一律寫 `noUpdate`，
  把「沒有東西可裝」當成「沒有更新版存在」，從不過問**有沒有能力檢查**。

  **這比沒有訊息更糟**：對一個根本收不到更新的使用者斬釘截鐵地說「你已經是最新版」，
  他不會有任何理由去懷疑。這也是 `BT-UPD-002` 能潛伏到 1.0.4 的原因之一 ——
  每個畫面都說一切正常，而它說謊的不是版本號，是「我查過了」這件事。

  兩支 status API 現在一律回報**當下**能不能檢查，不再沿用檢查當時存下來的值。

- **BT-UX-004** 更新卡片對一般使用者不可讀。

  原本畫面：`可更新至 1.0.1。 Beyblade Tracker · 26.2 MB · 2026-08-29T16:05:53.570Z Beyblade Tracker 1.0.1`

  原始 ISO 時間戳直接給了使用者；換行因為 `#update-details` 缺少 `white-space` 規則被吃掉，
  五段資訊擠成一句；發佈說明又重複了發行者。

  日期改為依介面語言格式化，換行實際生效，重複的發佈說明略過。
  日期無法解析時只丟掉日期，不會讓整張卡片壞掉。

### 已知未修

- `BT-UX-005`（P2）已載入的頁面不會反映後續變化（版本號、更新橫幅）。
- `BT-UX-007`（P2）更新**成功**後沒有使用者可及的回滾入口。
