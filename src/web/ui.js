...
document.getElementById('update-check')?.addEventListener('click',async()=>{setSettingsStatus(settingsMessages.checking);try{const data=await api('/api/update',{method:'GET'});setSettingsStatus(data.updateAvailable?data.manifest.version:settingsMessages.noUpdate,data.updateAvailable?'success':'');document.getElementById('update-apply').hidden=!data.updateAvailable}catch(error){setSettingsStatus(error.message,'error')}});
document.getElementById('update-apply')?.addEventListener('click',async()=>{setSettingsStatus(settingsMessages.checking);try{await api('/api/update/apply',{method:'POST',body:'{}'});setSettingsStatus(settingsMessages.saved,'success')}catch(error){setSettingsStatus(error.message,'error')}});
`;
}
