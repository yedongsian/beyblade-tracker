...
| 5 | **VM 的 A 段主流程（步驟 4～9）** | 乾淨 VM（`S1-with-chrome` 已拍） | 階段 2 已完成、階段 3 的安裝段已完成。**尚未跑**：導覽、五個捷徑、錯誤對話框、登出登入、兩次解除安裝。這些在路線 1 都已通過，但**乾淨 VM 上跑完整 A 段正是 RUNBOOK 第 13 節 clean VM gate 的內容**，因此該 gate 仍未滿足 |
