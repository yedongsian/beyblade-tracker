...
export function settingsScript(t = createTranslator()) {
  const messages = { saved: t('js.saved'), checking: t('settings.checking'), noUpdate: t('settings.noUpdate'), importConfirm: t('settings.importConfirm') };
  return `
...
const updateCard=document.getElementById('update-card');let availableUpdate=(()=>{try{return JSON.parse(updateCard?.dataset.scheduledUpdate||'null')?.manifest||null}catch{return null}})();const updateDetails=document.getElementById('update-details');const updateProgress=document.getElementById('update-progress');const updateApply=document.getElementById('update-apply');const updateDefer=document.getElementById('update-defer');
function bytes(value){return new Intl.NumberFormat(undefined,{maximumFractionDigits:1}).format(Number(value||0)/1048576)+' MB'}
function renderUpdate(data){availableUpdate=data.manifest||null;if(!data.updateAvailable||!availableUpdate){updateDetails.textContent=settingsMessages.noUpdate;updateApply.hidden=true;updateDefer.hidden=true;return}updateDetails.textContent='可更新至 '+availableUpdate.version+' · '+availableUpdate.publisher+' · '+bytes(availableUpdate.size)+' · '+availableUpdate.publishedAt+'\n'+availableUpdate.releaseNotes;updateApply.hidden=false;updateDefer.hidden=false}
if(availableUpdate)renderUpdate({updateAvailable:true,manifest:availableUpdate});setTimeout(async()=>{try{renderUpdate(await api('/api/update/status',{method:'GET'}))}catch{/* status is optional while the service starts */}},1000);setInterval(async()=>{try{renderUpdate(await api('/api/update/status',{method:'GET'}))}catch{/* keep the last verified result */}},60000);
async function pollUpdate(id){updateProgress.hidden=false;const timer=setInterval(async()=>{try{const data=await api('/api/update/progress/'+id,{method:'GET'});const percent=data.total?Math.min(100,Math.round(data.received/data.total*100)):0;updateProgress.value=percent;updateDetails.textContent=data.phase==='downloading'?'正在下載與驗證：'+percent+'%':data.phase==='installing'?'正在建立備份並啟動安裝器…':data.phase==='launched'?'安裝器已啟動，完成後將進行健康檢查。':data.error||'更新失敗。';if(['launched','failed'].includes(data.phase)){clearInterval(timer);updateProgress.hidden=true;setSettingsStatus(data.phase==='launched'?settingsMessages.saved:data.error,data.phase==='launched'?'success':'error')}}catch(error){clearInterval(timer);updateProgress.hidden=true;setSettingsStatus(error.message,'error')}},400)}
document.getElementById('update-check')?.addEventListener('click',async()=>{setSettingsStatus(settingsMessages.checking);try{const data=await api('/api/update',{method:'GET'});renderUpdate(data);setSettingsStatus(data.updateAvailable?'已驗證可用更新。':settingsMessages.noUpdate,data.updateAvailable?'success':'')}catch(error){setSettingsStatus(error.message,'error')}});
updateDefer?.addEventListener('click',async()=>{if(!availableUpdate)return;try{await api('/api/update/defer',{method:'POST',body:JSON.stringify({targetVersion:availableUpdate.version,manifestDigest:availableUpdate.manifestDigest})});setSettingsStatus('已選擇稍後更新。','success');updateApply.hidden=true;updateDefer.hidden=true}catch(error){setSettingsStatus(error.message,'error')}});
updateApply?.addEventListener('click',async()=>{if(!availableUpdate)return;if(!confirm('確認下載並安裝 '+availableUpdate.version+'？下載前不會啟動安裝器。'))return;setSettingsStatus(settingsMessages.checking);try{const data=await api('/api/update/apply',{method:'POST',body:JSON.stringify({confirmed:true,targetVersion:availableUpdate.version,manifestDigest:availableUpdate.manifestDigest})});updateApply.hidden=true;updateDefer.hidden=true;pollUpdate(data.operationId)}catch(error){setSettingsStatus(error.message,'error')}});
`;
}
