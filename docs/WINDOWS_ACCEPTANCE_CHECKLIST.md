...
SHA-256：**`2e11a4bd5fb450e7ad12c7f1b7a0da1ae7cf9731b0c78b34340f0e225760c05f`**（27,513,416 bytes，2026-08-11 第七次建置）
建立日期：2026-08-02（第一輪）／2026-08-06（第二、三輪）／2026-08-07（第四輪）／2026-08-11（第五、六輪）
...
| **第七輪** | **`2e11a4bd…`** | 修正冷卻誤報 `BT-LCH-999`（`BT-API-001` 首片）與 Recipe 行未翻譯 | **現行驗收對象** |
...
> **第六版的建置端驗證**：來源 `main` @ `c868fbe`；單元測試 **249/249**；release E2E 四項全綠（normal／stopfail／missing-launcher／launcher-errors **6-6**）；8787 淨空、無 temp 殘留、無殘留行程。
...
| 2 | **附帶發現二：操作回饋在視野外** | 程式修改 | 「立即重新檢查」「探索商品」的回饋寫到頁面頂端的單一狀態列，在下方卡片操作時看不到 → 看起來像沒反應 → 連按 → **這正是 D-8 的觸發源**。修掉它等於從源頭減少 D-8 的發生 |
| 3 | **附帶發現一：dns 建議語會誤導** | 程式修改 | 單純斷網會分類成 `dns`，於是顯示「請確認網址拼寫」——但該來源先前明明成功過。筆電斷線是最常見情境，這句話會把人導向錯誤方向。建議：曾成功過的來源改用連線相關建議 |
| 4 | **今天三項修正的實機複驗** | 第七版產物 | D-8 冷卻改回報 `BT-SRC-003`／409、Recipe 行三語化、`fetch failed`／`too_large` 分類。三者都只有自動化測試，尚未在產物上看過 |
| 5 | **VM 階段 2 ＋ 階段 3** | 乾淨 VM（S0 快照仍在） | 還原 `S0-no-chrome` → 裝 Chrome → 拍 `S1-with-chrome` → 跑 A 段主流程步驟 1～9。**這才真正滿足 RUNBOOK 第 13 節的 clean VM gate** —— 路線 1 從未滿足過，VM 輪目前也只跑了階段 1、4、5 |
